...
for j in range(len(bandwidth)):
    ...
    for i in range(len(group)):
        ...
        for k in range(len(telescope)):
            ...
            for p in range(len(stats)):
                s = stats[p]
                signal = mean_stats[s]
                ax[p, i].plot(x, signal, ls=linestyles[k], c=colors[k])
                if p > 0:
                    ax[p, i].axhline(0, c='k', ls='--', lw=1)
    ...
    fig.savefig(
        '/Users/piyanat/Google/research/hera1p/plots/averaged_stats_heraxx/'
        'averaged_stats_heraxx_bw{:.2f}MHz.pdf'.format(b)
    )
