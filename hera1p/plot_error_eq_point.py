    ...
    for j in range(len(bandwidth)):
        ...
        for k in range(len(telescope)):
            ...
            for l in range(len(stats)):
                s = stats[l]
                gt_eqp_idx = x[np.where(sample_err[s] > noise_err[s])].values
                if gt_eqp_idx.size > 0:
                    if g == 'binning':
                        ax.scatter(k+(j*0.1), gt_eqp_idx[0], s=markersize[j], c=colors[l], marker=markers[l], alpha=0.5, edgecolors='face')
                    else:
                        ax.scatter(k+(j*0.1), gt_eqp_idx[0], s=markersize[j], c='none', marker=markers[l], alpha=0.5, edgecolors=colors[l])
