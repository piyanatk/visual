...
for i in range(int(200 / nf)):  # Pages
    ...
    plt.close()
    fig, ax = plt.subplots(5, nf, sharex='all', sharey='all', figsize=(13, 7.5))
    for j in range(5):  # Rows, bandwidths
        bw = bandwidth[j]
        pn1 = pd.read_hdf(
        ...
        for k in range(nf):  # Columns, fields
            ...
            if k == nf-1:
                ax[j, k].set_ylabel('{:.2f} MHz'.format(bw))
    ...
    fig.savefig('/Users/piyanat/Google/research/hera1p/plots/'
                'mc_stats_bw0.08-4.00MHz/{:s}/{:s}/'.format(tl, st) +
                'mc_stats_bw0.08-4.00MHz_{:s}_{:s}_field{:03d}-{:03d}.pdf'
                .format(tl, st, pt[0], pt[-1]))
