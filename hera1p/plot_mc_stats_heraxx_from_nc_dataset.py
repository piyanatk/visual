...
for i in range(npages):
    ...
    for j in range(nrows):  # Rows, telescopes
        for k in range(ncols):  # Columns, fields
            f = ncols * i + k
            cuts1 = dict(telescope=j, field=f, stat=2, averaging=1)
            cuts2 = dict(telescope=j, stat=2, averaging=1)
            ...
            l2 = ax[j, k].plot(x, ymean, color='black', linestyle=':',
                               alpha=0.8, zorder=2)
            s1 = ax[j, k].fill_between(x, yerr_min, yerr_max, edgecolors='None',
    ...
    fig.subplots_adjust(left=0.05, right=0.96, bottom=0.08, top=0.95,
                        hspace=0, wspace=0)
    outdir = main_dir + '/plot_v2/mc_stats_heraxx/bw{:.2f}MHz/kurt'\
        .format(bandwidth)
