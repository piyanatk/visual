...
for j in range(len(bandwidth)):
    b = bandwidth[j]
    plt.close()
    fig, ax = plt.subplots(len(stats), 2, sharex='all', sharey='row',
                           figsize=(8.5, 5.5))
    for i in range(len(group)):
        ...
        for k in range(len(telescope)):
            ...
            for p in range(len(stats)):
                ...
                ax[p, i].plot(x, snr, ls=linestyles[k], c=colors[k])
                ax[p, i].axhline(1, c='k', ls='--', lw=1)
    ...
    fig.legend(handles=handles, labels=labels, ncol=9, loc='upper center',
               handlelength=3.7, fontsize='x-small')

    fig.subplots_adjust(left=0.08, right=0.97, bottom=0.09, top=0.87,
                        hspace=0, wspace=0)

    fig.savefig(
        '/Users/piyanat/Google/research/hera1p/plots/snr_heraxx/'
        'snr_heraxx_bw{:.2f}MHz.pdf'.format(b)
    )
